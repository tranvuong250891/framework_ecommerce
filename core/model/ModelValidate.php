...
abstract class ModelValidate
{
    ...
    {
        $rules = $this->rules();
        foreach (array_keys($data) as $attr) { $this->values[$attr] = trim($data[$attr]);}
        return $rules;
    }
    ...
    private function addErrorForRule(string $attr, string $rule, $params = [])
    {
        ...
        foreach($params as $k =>$v){ $mess = str_replace("{{$k}}", $v, $mess); }
    ...
    public function statement($sql, $where = [])
    {
        ...
        if(!is_array($where)) { $statement->execute(); return $statement; } else {
