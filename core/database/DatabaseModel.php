...
use app\core\App;
use app\core\model\ModelValidate;
use app\core\Test;
...
abstract class DatabaseModel extends ModelValidate
{
    ...
    public function getValue($data)
    {
        ...
        foreach ($this->matchTable() as $match) {
            ...
            } else { $set[$match['get']] =  $fetch[$match['get']]; }
        }
        return $set ?? [];
    }
    ...
    {
        $sql = 'SELECT * FROM ' . $this->getNameDb($table) . ' WHERE ' .$this->strWhere($where);
    ...
    public function find($where, $orderby = 'id', $limit = "all", $offset = 0)
    {
        ...
        if($where === 'all'){ $sql = 'SELECT * FROM ' . $this->getNameDb() . $setSql;
        } else{ $sql = 'SELECT * FROM ' . $this->getNameDb() . ' WHERE ' .$this->strWhere($where). $setSql;}
    ...
    public function save()
    {
        ...
        foreach($this->attributes() as $attr){ $where[$attr] = $this->{$attr};};
    ...
    public function update($where = [], $set = [], $table = null)
    {
        ...
        foreach($set as $attr){ $data[$attr] = $this->{$attr}; }
        $data =  $where + $data;
